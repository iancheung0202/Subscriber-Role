...
async def init_db():
    ...
    async with _init_lock:
        ...
        async with pool.acquire() as connection:
            await connection.execute("CREATE TABLE IF NOT EXISTS servers (id SERIAL PRIMARY KEY, guild_id BIGINT NOT NULL, yt_channel_id TEXT NOT NULL, role_id BIGINT NOT NULL, log_channel_id BIGINT)")
...
async def set_server_config(guild_id: int, yt_channel_id: str, role_id: int, log_channel_id: int = None, server_id: int = None):
    ...
    async with pool.acquire() as connection:
        if server_id:
            await connection.execute("UPDATE servers SET yt_channel_id = $1, role_id = $2, log_channel_id = $3 WHERE id = $4 AND guild_id = $5", yt_channel_id, role_id, log_channel_id, server_id, guild_id)
            return server_id
        else:
            new_id = await connection.fetchval("INSERT INTO servers (guild_id, yt_channel_id, role_id, log_channel_id) VALUES ($1, $2, $3, $4) RETURNING id", guild_id, yt_channel_id, role_id, log_channel_id)
            return new_id

async def get_all_server_configs():
    pool = await get_pool()
    async with pool.acquire() as connection:
        return await connection.fetch("SELECT id, guild_id, yt_channel_id, role_id, log_channel_id FROM servers ORDER BY guild_id, id")
...
async def get_server_config_by_id(server_id: int):
    pool = await get_pool()
    async with pool.acquire() as connection:
        return await connection.fetchrow("SELECT id, guild_id, yt_channel_id, role_id, log_channel_id FROM servers WHERE id = $1", server_id)
...
async def get_server_configs_for_guild(guild_id: int):
    pool = await get_pool()
    async with pool.acquire() as connection:
        return await connection.fetch("SELECT id, guild_id, yt_channel_id, role_id, log_channel_id FROM servers WHERE guild_id = $1 ORDER BY id", guild_id)
