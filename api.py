...
def generate_page(title: str, description: str, url: str, body_html: str, centered: bool = False) -> str:
    if centered:
        styles = "body { font-family: sans-serif; max-width: 800px; margin: 40px auto; padding: 0 20px; line-height: 1.6; text-align: center; } a { color: #af4875; border: 2px solid #d8c3cd; padding: 8px 12px; border-radius: 8px; text-decoration: none; display: inline-block; margin: 2px; transition: all 0.3s ease; } a:hover { opacity: 0.85; transform: translateY(-2px); box-shadow: 0 2px 8px rgba(0,0,0,0.15); } a:active { transform: translateY(0); } footer { text-align: center; margin-top: 40px; padding-top: 20px; border-top: 1px solid #ccc; font-size: 0.9em; color: #666; }"
    else:
        styles = "body { font-family: sans-serif; max-width: 800px; margin: 40px auto; padding: 0 20px; line-height: 1.6; } a { color: #af4875; border: 2px solid #d8c3cd; padding: 8px 12px; border-radius: 8px; text-decoration: none; display: inline-block; margin: 2px; transition: all 0.3s ease; } a:hover { opacity: 0.85; transform: translateY(-2px); box-shadow: 0 2px 8px rgba(0,0,0,0.15); } a:active { transform: translateY(0); } footer { text-align: center; margin-top: 40px; padding-top: 20px; border-top: 1px solid #ccc; font-size: 0.9em; color: #666; }"
    footer_html = '<footer><p>Developed by Ian Cheung • All rights reserved.</p></footer>'
    full_body = f'{body_html}\n        {footer_html}'
    return f'<!DOCTYPE html> <html lang="en"> <head> <meta charset="UTF-8"> <meta name="viewport" content="width=device-width, initial-scale=1.0"> <meta name="description" content="{description}"> <meta name="theme-color" content="#af4875"> <meta property="og:title" content="{title}"> <meta property="og:description" content="{description}"> <meta property="og:image" content="{BOT_AVATAR}?size=256"> <meta property="og:url" content="{url}"> <meta property="og:type" content="website"> <meta name="twitter:card" content="summary_large_image"> <meta name="twitter:title" content="{title}"> <meta name="twitter:description" content="{description}"> <meta name="twitter:image" content="{BOT_AVATAR}?size=256"> <title>{title}</title> <link rel="icon" href="{BOT_AVATAR}?size=32" type="image/png"> <style>{styles}</style> </head> <body> {full_body} </body> </html>'
...
async def validate_request_size_and_user_agent(request: Request, call_next):
    ...
    if not any(crawler in user_agent for crawler in os.getenv("ALLOWED_CRAWLERS").split(",")):
        if any(blocked in user_agent for blocked in os.getenv("BLOCKED_USER_AGENTS").split(",")):
...
async def home_page(request: Request):
    return generate_page("Subscriber Role - Discord Bot", "Verify YouTube channel subscriptions and automatically assign Discord roles with Subscriber Role bot.", "https://subscriber.iancheung.dev/",
...
async def privacy_policy(request: Request):
    return generate_page("Subscriber Role - Privacy Policy", "Privacy policy for Subscriber Role Discord bot. Learn how we handle your data.", "https://subscriber.iancheung.dev/privacy",
        f"""<h1>Privacy Policy for Subscriber Role</h1>
        ...
        <p><strong>Important:</strong> We only access to view your subscription data. We do <strong>NOT</strong> access your email address, password, personal contact information, watch history, private videos or playlists, YouTube comments, interactions, or any other Google services or data not explicitly listed above.</p>
...
async def terms_of_service(request: Request):
    return generate_page("Subscriber Role - Terms of Service", "Terms of service for Subscriber Role Discord bot. Read our usage policies and terms.", "https://subscriber.iancheung.dev/terms",
        f"""<h1>Terms of Service for Subscriber Role</h1>
        <p><a href="/">Return to Home</a></p>
        <p>Last updated: {datetime.datetime.now().strftime("%B %d, %Y")}</p>
...
def check_subscriptions(credentials, channel_id: str):
    try:
...
def verify_signature(state: str, state_secret: str) -> tuple:
    try:
        if '.' not in state:
            return None
...
def render_response(title: str, content: str) -> str:
    return generate_page(f"Subscriber Role - {title}", content[:160], "https://subscriber.iancheung.dev/callback", f"""<h1>{title}</h1> <p>{content}</p> <p><a href="/">Return to Home</a></p>""", centered=True)
...
async def callback(request: Request):
    ...
    if not code or not state:
        return HTMLResponse(render_response("Invalid Request", "Missing code or state."))
    state_secret = os.environ.get("STATE_SECRET")
    if not state_secret:
        return HTMLResponse(render_response("Configuration Error", "Server configuration error. Please try again later."))
    parsed_state = verify_signature(state, state_secret)
    if not parsed_state:
        return HTMLResponse(render_response("Invalid Request", "Invalid or tampered state parameter. Please try verification again."))
    guild_id, discord_id, server_id = parsed_state
    server_config = await get_server_config_by_id(server_id)
    if not server_config:
        return HTMLResponse(render_response("Configuration Error", "This server hasn't been configured yet. Ask an admin to run `/setup`."))
    ...
    if "error" in token_response:
        return HTMLResponse(render_response("Authentication Error", f"Error authenticating: {token_response.get('error_description', 'Unknown error')}"))
    ...
    if not refresh_token:
        ...
        async with pool.acquire() as connection:
            ...
            if row and row['refresh_token']:
                ...
                return HTMLResponse(render_response("Access Denied", "Could not retrieve a refresh token. Please re-authenticate and make sure to allow offline access."))
    else:
    ...
    is_subscribed = await asyncio.to_thread(check_subscriptions, creds, yt_channel_id)
    await update_subscription_status(user_id, server_id, yt_channel_id, is_subscribed)
    guild = bot.get_guild(guild_id)
    if not guild:
        return HTMLResponse(render_response("Bot Error", "Internal bot error: Cannot find Discord server."))
    try:
        member = await guild.fetch_member(discord_id)
    except discord.errors.NotFound:
        return HTMLResponse(render_response("User Not Found", "Cannot find your user in the Discord server. Are you sure you are in it?"))
    except discord.errors.Forbidden:
        return HTMLResponse(render_response("Permission Error", "Bot lacks permission to fetch users from the Discord server. (Is Server Members intent enabled?)"))
    except Exception as e:
        return HTMLResponse(render_response("Unexpected Error", f"An unexpected error occurred while fetching your Discord profile: {e}"))
    role = guild.get_role(role_id)
    if not role:
        return HTMLResponse(render_response("Role Not Found", "Internal bot error: Cannot find the specified role."))
    ...
    if is_subscribed:
        try:
            ...
            try:
                ...
                pass
            return HTMLResponse(render_response("Permission Error", "Bot lacks permission to assign this role. Please explicitly ensure the bot's role is higher in the hierarchy than the Subscriber role."))
        except Exception as e:
            return HTMLResponse(render_response("Unexpected Error", f"An unexpected error occurred while assigning the role: {e}"))
        ...
        try:
            ...
            return HTMLResponse(render_response("Success", f"Success! However, the bot failed to send a log message to the log channel. Please check the log channel permissions. Error: {e}"))
        return HTMLResponse(render_response("Verification Successful", "Success! Your subscription has been verified and you have been granted the role on Discord. You may now close this window."))
    else:
        try:
            ...
            pass
        return HTMLResponse(render_response("Verification Failed", "Verification failed: You are not subscribed to the required channel. Please subscribe and try again."))
